  ...
  render() {
    ...
    return (
      <div>
        ...
        {/* conditional render for bolean value */}
        { this.props.showTitle === true && (
          <h1>{this.props.title}</h1>
        )}

        {/* conditional render for bolean value */}
        { (this.props.showTitle === true || this.props.title.length > 0) && (
          <h1>{this.props.title}</h1>
        )}

         {/* conditional render for bolean value */}
         {this.props.showTitle === true && this.props.title.length > 0 && (
          //  can put a component here
          <h1>{this.props.title}</h1>
        )}

        {/* conditional render with length must have > 0 for the expression */}
        { this.props.title.length > 0 && (
          <h1>{this.props.title}</h1>
        )}

        { /* const myname = (name.length) ? "gary" : "fred" */}
        {this.props.showTitle === true && this.props.title.length > 0 ? (
          //  can put a component here
          <h1>{this.props.title}</h1>
        ) : (
